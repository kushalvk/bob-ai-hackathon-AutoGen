...
class CAPAReport(Base):
    ...
    Attributes:
        capa_id (str): Primary key unique identifier (e.g. 'CAPA-001').
        deviation_ids (list[str]): List of associated deviation primary key IDs.
        root_cause (str): Narrative analysis of underlying failure root cause.
        corrective_action (str): Action taken to address immediate deviation.
        preventive_action (str): Long-term process changes implemented to prevent recurrence.
    ...
    def __repr__(self) -> str:
        return f"<CAPAReport(id='{self.capa_id}', owner='{self.owner}', status='{self.status}')>"
